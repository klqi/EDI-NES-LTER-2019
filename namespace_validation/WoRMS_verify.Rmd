...
is_abiotic <- function(name, counter) {
  if (name == "detritus" | name == "bead" | name == "bubble" | name == "pollen" | name == "camera spot") {
    man.data$resolved_id[counter] <<- -999999
...
is_specific <- function(name, counter) {
  ...
  # hard coding for Dictyocha
  # if (name == "Dictyocha") {
  #   # set as Dictyocha Ehrenberg
  #   man.data$resolved_id_fromgnr[counter] <<- 157258
  #   return(FALSE)
  # }
  return(FALSE)
}
...
is_valid <- function(resolved_name) {
  ...
  err <- FALSE
  # check records
  tryCatch( {stats <- wm_records_name(name = check) },
            error = function(e) {err <<- TRUE})
  # check species doesn't exist on worms
  if (err == TRUE | nrow(stats) > 1) {
    # just return current name
    return(check)
  } else {
    # if stats is only one long
