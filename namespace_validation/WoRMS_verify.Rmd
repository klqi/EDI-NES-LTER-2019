...
for (row in 1:nrow(man.data)) {
  ...
  if (!empty(temp)) {
    ...
    # gnr unable to resolve by name, fill with NA 
    ...
    if(!is.na(man.data$international_id[counter])) {
      # set taxon name from international id
      taxon_name <- wm_id2name(id=man.data$international_id[counter])
      man.data$taxon_name[counter] <- taxon_name
      # check if name returned from international id is valid 
      if (taxon_name == man.data$name[counter]) {
        ...
        if (!empty(response)) {
          ...
          man.data$resolved_id[counter] <- TRUE
          man.data$data_source[counter] <- "World Register of Marine Species"
        }
      }
    } else {
